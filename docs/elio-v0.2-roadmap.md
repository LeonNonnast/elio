...
| **Vela suspend/resume** | ✅ done (Double-verifiziert) | Genereller Core-Seam: die Resume-Antwort reist über `ctx.resume` → `SessionContract.resume` zur Engine (der Runner setzt sie NUR am wieder-ausgeführten suspendierten Step). Vela-Bridge: persistenter (turn-übergreifender) Store + resume-stabiler Identity-Key (`run::branch::step`, ohne volatile checkpoint) → `findByIdentity` re-findet den pausierten Run, `advance({resumeAnswer})` unblockt. Voller Runner-Roundtrip (suspend → resume → resolve) gegen das faithful Double getestet. Ehrlich: reales Vela pausiert im Single-Delegate-Step-Shape (noch) nicht — ein Multi-Step/Pause-Surface-Workflow ist die verbleibende Real-Path-Arbeit. |
